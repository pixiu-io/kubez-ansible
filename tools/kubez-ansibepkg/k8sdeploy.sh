...
function add_hosts() {
    ...
    grep -q "$mirrors_repo_infor" /etc/hosts || echo "$variable $mirrors_repo_infor" >> /etc/hosts
    grep -q "$regis_repos_infor"  /etc/hosts || echo "$variable $regis_repos_infor"  >> /etc/hosts
}
